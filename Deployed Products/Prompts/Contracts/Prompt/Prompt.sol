...
import "./PromptManager.sol";
import "./IPromptManager.sol";

/**
* @title Prompt
* @dev This contract represents a prompt that users can deploy and subscribe to.
*/
contract Prompt is ERC1155, ERC1155URIStorage, ReentrancyGuard, AccessControl {
   using SafeERC20 for IERC20;

   /**
    * @dev Struct to hold the price configuration for a prompt.
    * @param token The ERC20 token used for payment.
    * @param price The price of the prompt in the specified token.
    */
   struct Price {
       IERC20 token;
       uint96 price;
   ...
   mapping(uint256 => IERC20[]) public tokensForPrompt;

   event DeployPrompt(address indexed creator, string hash, uint256 tokenId);
    ...
    * @param _name The product token name.
    * @param _symbol The product token symbol.
    */
   constructor(address _ProductManager, string memory _ProductUri, string memory _name, string memory _symbol) ERC1155(_ProductUri) {
       config = IPromptManager(_ProductManager);
       promptCounter = 1;
    ...
    /**
     * @notice Ensures the contract is not paused in the PromptManager.
     * @dev Checks the `isPaused` function in the PromptManager contract.
     */
    modifier whenNotPaused() {
        require(!config.isPaused(), "Prompt actions are currently paused");
        _;
    }

/**
 * @notice Deploys a new prompt by creating a new token and assigning it a unique hash and URI.
 ...
 * @param promptHash Unique identifier for the prompt.
...
function deployPrompt(
    ...
    uint256 tokenId = promptCounter++;
    tokenToHash[tokenId] = promptHash;
    hashToToken[promptHash] = tokenId;

    // Set prices and add tokens to list for the new prompt
    for (uint256 i = 0; i < prices.length; i++) {
        ...
        if (!isTokenInList(tokenId, prices[i].token)) {
            tokensForPrompt[tokenId].push(prices[i].token);
        }
    }
    ...
    emit DeployPrompt(msg.sender, promptHash, tokenId);
...
function updatePrice(
    ...
    delete tokensForPrompt[tokenId];
    ...
    for (uint256 i = 0; i < newPrices.length; i++) {
        ...
        // Update the price mapping and add the token to tokensForPrompt list
        price[tokenId][newPrices[i].token] = newPrices[i].price;
        tokensForPrompt[tokenId].push(newPrices[i].token);
    ...
    function getTokenPrices(uint256 tokenId) external view returns (Price[] memory) {
        IERC20[] memory tokens = tokensForPrompt[tokenId];
...
function subscribe(IERC20 paymentToken, uint256 tokenID) external nonReentrant whenNotPaused {
   ...
   function setProductURI(string memory newURI) external onlyRole(MANAGER_ROLE) {
    ...
    /**
     * @notice Checks if an ERC20 token is already in the list of tokens for a specific prompt ID.
     * @param tokenId The ID of the prompt.
    ...
    /**
 * @notice Updates the PromptManager address for configuration settings.
 * @dev This function allows the admin to change the PromptManager address, in case it needs to be updated.
 * @param newConfigAddress The address of the new PromptManager contract.
 */
function setConfigAddress(address newConfigAddress) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    config = IPromptManager(newConfigAddress);
}
}
